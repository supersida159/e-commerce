...
import (
	...
	"github.com/supersida159/e-commerce/update-inventory-service/pkg/app_context"
	"github.com/supersida159/e-commerce/update-inventory-service/pkg/config"
	dbs "github.com/supersida159/e-commerce/update-inventory-service/pkg/db"
	"github.com/supersida159/e-commerce/update-inventory-service/pkg/kafka/consumerlocal"
	kafkaconfig "github.com/supersida159/e-commerce/update-inventory-service/pkg/kafka/kafka_config"
	"github.com/supersida159/e-commerce/update-inventory-service/pkg/kafka/producers"
	"github.com/supersida159/e-commerce/update-inventory-service/pkg/localredis"
	"github.com/supersida159/e-commerce/update-inventory-service/pkg/pubsub/pubsublocal"
	"github.com/supersida159/e-commerce/update-inventory-service/pkg/subscriber"
	entities "github.com/supersida159/e-commerce/update-inventory-service/src/model"
	"github.com/supersida159/e-commerce/update-inventory-service/src/repository"
	"github.com/supersida159/e-commerce/update-inventory-service/src/usecase"
)
...
func main() {
	...
	Store := repository.NewSQLStore(db.GetDB())
	...
	consumer, err := consumerlocal.NewSagaConsumer(cfg, cfg.Kafka.Brokers, kafkaconfig.InventoryService)
	if err != nil {
	...
	inventoryUseCase := usecase.NewInventoryUsecase(Store)

	// Initialize subscriber
	sub := subscriber.NewSubscriber(appCtx)

	// Register handlers update inventory
	sub.RegisterHandler(consumerlocal.CreateOrderChannel, func(ctx context.Context, data *entities.OrderEvent) *common.AppError {
		result := inventoryUseCase.UpdateInventory(ctx, *data.Cart)
		data.CurrentService = entities.InventoryService
		if result != nil {

			data.ServiceStatus.LastUpdated = time.Now()
			data.ServiceStatus.ServiceStates[entities.InventoryService] = entities.ServiceState{
	...
	sub.RegisterHandler(consumerlocal.RollbackChannel, func(ctx context.Context, data *entities.OrderEvent) *common.AppError {
		...
		result := inventoryUseCase.CompensateInventory(ctx, *data.Cart)
		data.CurrentService = entities.InventoryService
